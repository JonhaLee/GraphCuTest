...
double my::GMM::operator()(const Vec3d color) const
{
	double res = 0;
	for (int ci = 0; ci < componentsCount; ci++)
...
double my::GMM::operator()(int ci, const Vec3d color) const
{
	double res = 0;
	if (coefs[ci] > 0)
	{
...
int my::GMM::whichComponent(const Vec3d color) const
{
	int k = 0;
	double max = 0;
...
void my::GMM::addSample(int ci, const Vec3d color)
{
...
void my::GMM::calcInverseCovAndDeterm(int ci)
{
	if (coefs[ci] > 0)
	{
		double *c = cov + 9 * ci;
...
*/
static void initGMMs(const Mat& img, const Mat& mask, my::GMM& bgdGMM, my::GMM& fgdGMM)
{
	...
	kmeans(_bgdSamples, my::GMM::componentsCount, bgdLabels,
		TermCriteria(CV_TERMCRIT_ITER, kMeansItCount, 0.0), 0, kMeansType);
	Mat _fgdSamples((int)fgdSamples.size(), 3, CV_32FC1, &fgdSamples[0][0]);
	kmeans(_fgdSamples, my::GMM::componentsCount, fgdLabels,
		TermCriteria(CV_TERMCRIT_ITER, kMeansItCount, 0.0), 0, kMeansType);
...
*/
static void assignGMMsComponents(const Mat& img, const Mat& mask, const my::GMM& bgdGMM, const my::GMM& fgdGMM, Mat& compIdxs)
{
	Point p;
	for (p.y = 0; p.y < img.rows; p.y++)
	{
...
*/
static void learnGMMs(const Mat& img, const Mat& mask, const Mat& compIdxs, my::GMM& bgdGMM, my::GMM& fgdGMM)
{
	bgdGMM.initLearning();
	fgdGMM.initLearning();
	Point p;
	for (int ci = 0; ci < my::GMM::componentsCount; ci++)
	{
		for (p.y = 0; p.y < img.rows; p.y++)
		{
			for (p.x = 0; p.x < img.cols; p.x++)
